...
// Directs the serial port transmit and receive to RN2483
...
// Directs the serial port transmit and receive to USB
...
// Gets the battery voltage from the RN2483
// returns voltage in mV
uint16_t Sodaq_RN2483::getBatVolts(void)
{
	uBit.serial.send("[getBatVolts]", SYNC_SPINWAIT);
    
	serialToRN2483(); // UART now pointing to RN2483
	uBit.serial.send(STR_CMD_GET_BATVOLTS, SYNC_SPINWAIT);
	uBit.serial.send(CRLF, SYNC_SPINWAIT);	
    ...
    while (uBit.systemTime() < start + DEFAULT_TIMEOUT) {
        ...
        if (uBit.serial.rxBufferedSize() != 0){
			readChar = uBit.serial.read();
			if( readChar == '\n'){
				for( int i=0; i < inputIndex -1; i++)
					batmV = batmV * 10 +(inputBuffer[i] - '0');
			
				serialToUSB();
				uBit.serial.send("BatmV: ", SYNC_SPINWAIT);
				uBit.serial.send(this->inputBuffer, SYNC_SPINWAIT);
				uBit.serial.send(CRLF, SYNC_SPINWAIT); 
				return batmV;
			}
	...
	serialToUSB();
    uBit.serial.send("[getBatVolts] Timed out without a response!", SYNC_SPINWAIT);
    return 0;
}
